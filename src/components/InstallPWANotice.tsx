    ...
    return (
      <div className="fixed right-0 top-[42dvh] z-[260] md:hidden" dir="rtl">
        ...
        >
          ...
          <span className="material-symbols-outlined text-base text-[var(--pm-primary)] transition-transform group-hover:-translate-x-0.5">chevron_left</span>
        </button>
      </div>
    );
  }
      ...
      <section className="motion-pop flex max-h-[calc(100dvh-1.5rem)] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-white/15 bg-white shadow-2xl shadow-black/50 dark:bg-zinc-950 sm:max-h-[calc(100dvh-2rem)]">
        <header className="pm-contrast-surface shrink-0 border-b border-[var(--pm-line)] bg-zinc-950 px-4 py-3 text-white">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--pm-primary)]">{pwa.guide.platformLabel} PWA</p>
            ...
            <button
              type="button"
              onClick={close}
              className="flex size-9 shrink-0 items-center justify-center rounded-lg border border-[var(--pm-line)] bg-white/10 text-white transition-all hover:bg-white hover:text-zinc-950"
          ...
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            {installBenefits.map((item) => (
              <span key={item.title} className="rounded-lg border border-[var(--pm-line)] bg-white/10 p-2 text-center">
                <span className="material-symbols-outlined text-base text-[var(--pm-primary)]">{item.icon}</span>
                <span className="mt-0.5 block text-[10px] font-black text-white">{item.title}</span>
                <span className="mt-0.5 block truncate text-[9px] font-bold text-[var(--pm-muted)]">{item.text}</span>
              </span>
            ))}
          </div>
        </header>
        ...
        <div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto px-3 py-3">
          <section className="overflow-hidden rounded-xl border border-cyan-500/25 bg-[var(--pm-primary)]/10">
            <div className="flex items-center justify-between gap-3 border-b border-[var(--pm-primary)]/20 bg-white/80 p-2.5 dark:bg-zinc-950/55">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-black text-[var(--pm-primary)]">راهنمای دقیق نصب</p>
                  <h3 className="mt-0.5 truncate text-sm font-black text-[var(--pm-text)]">{pwa.guide.label}</h3>
                </div>
              </div>
            ...
            <ol className="grid gap-1.5 p-2.5">
              {pwa.guide.steps.map((step, index) => (
                <li key={step} className="grid grid-cols-[2.25rem_minmax(0,1fr)] gap-2 rounded-lg border border-[var(--pm-line)] bg-white p-2 shadow-sm shadow-zinc-950/5 dark:border-[var(--pm-line)] dark:bg-zinc-950/70">
                  <span className="relative flex size-9 items-center justify-center rounded-lg bg-[var(--pm-primary)] text-[var(--pm-text-inverse)]">
          ...
          <details className="group mt-2 overflow-hidden rounded-lg border border-sky-500/20 bg-sky-500/10">
            <summary className="flex min-h-10 cursor-pointer list-none items-center justify-between gap-3 px-3 text-xs font-black text-[var(--pm-text)]">
              <span className="flex items-center gap-2">
                <span className="material-symbols-outlined text-base text-sky-600 dark:text-sky-300">tips_and_updates</span>
                بعد از نصب
              </span>
              <span className="material-symbols-outlined text-lg text-[var(--pm-muted)] transition-transform group-open:rotate-180">keyboard_arrow_down</span>
            </summary>
          ...
          <p className="mt-2 rounded-lg border border-[var(--pm-line)] bg-zinc-50 p-2.5 text-[11px] font-bold leading-5 text-[var(--pm-muted)] dark:border-[var(--pm-line)] dark:bg-white/[0.03] dark:text-[var(--pm-muted)]">
            {pwa.guide.note}
          </p>
        </div>

        <footer className="shrink-0 border-t border-[var(--pm-line)] bg-white p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] dark:border-[var(--pm-line)] dark:bg-zinc-900/95">
          <div className="grid gap-2">
            {installPrompt && (
              <button type="button" onClick={install} className="pm-button-primary min-h-11 w-full">
                <span className="material-symbols-outlined text-xl">download</span>
                نصب مستقیم برنامه
              </button>
            )}
            <button type="button" onClick={close} className="pm-button-secondary min-h-10 w-full">
              بستن تا ورود بعدی
            </button>
          </div>
        </footer>
